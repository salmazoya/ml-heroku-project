...
def train_test_model():
    ...
    data = pd.read_csv(DATA_PATH+'cleaned_dataset.csv')
    ...
    joblib.dump(trained_model, MODEL_PATH+'trained_model.joblib')
...
def check_score():
    ...
    """
    df = pd.read_csv(DATA_PATH+'cleaned_dataset.csv')
    _,test = train_test_split(df,test_size=0.2)

    trained_model = joblib.load(MODEL_PATH+'trained_model.joblib')
    ...
    for cat in get_cat_features():
        for cls in test[cat].unique():
            ...
            X_test,y_test,_,_ = process_data(df_temp,categorical_features=get_cat_features(),
    ...
    with open('../model/slice_scores.txt', 'w') as out:
